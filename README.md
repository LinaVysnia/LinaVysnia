...
### [Architectural Intelligence](https://github.com/LinaVysnia/Intelligent-Architecture)
Can a machine read a floor plan the way an architect does? This project attempts to find out. A YOLOv8 instance segmentation model trained to detect and classify rooms in residential floor plans — bedrooms, kitchens, bathrooms, living rooms, and more — then calculates their areas and checks them against Lithuanian construction regulations (STR 2.02.01:2004). The result: an automated compliance report that flags rooms that are too small, too cramped, or otherwise non-conforming. Training involved experimenting across multiple model sizes and epoch counts to find the sweet spot between accuracy and speed, all done on consumer-grade hardware with a healthy amount of perseverance.
